...
namespace RealtimeCSG.Helpers
{
	public sealed partial class CSGHandles
	{
		...
		public static float ScaleValueHandle(float value, Vector3 position, Quaternion rotation, float size, CapFunction capFunction, bool snapping, float snap, CSGHandles.InitFunction initFunction, CSGHandles.InitFunction shutdownFunction)
		{
		...
		public static Vector3 ScaleHandle(Vector3 scale, Vector3 position, Quaternion rotation, bool snapping, CSGHandles.InitFunction initFunction, CSGHandles.InitFunction shutdownFunction)
		{
			...
			var oldScaleValue = scale.x;
			var newScaleValue = ScaleValueHandle(oldScaleValue, position, rotation, size, CubeHandleCap, snapping, RealtimeCSG.CSGSettings.SnapScale, initFunction, shutdownFunction);
			if (EditorGUI.EndChangeCheck())
			{
				float dif = newScaleValue / oldScaleValue;
				if (!RealtimeCSG.CSGSettings.LockAxisX) scale.x = newScaleValue;
				if (!RealtimeCSG.CSGSettings.LockAxisY) scale.y *= dif;
				if (!RealtimeCSG.CSGSettings.LockAxisZ) scale.z *= dif;
			}
