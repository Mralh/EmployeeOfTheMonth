...
namespace RealtimeCSG.Helpers
{
	public class CSGScaleSlider
	{
		...
		public static float DoCenter(int id, float value, Vector3 position, Quaternion rotation, float size, CSGHandles.CapFunction capFunction, bool snapping, float snap, CSGHandles.InitFunction initFunction, CSGHandles.InitFunction shutdownFunction)
		{
			size *= 0.5f;
			var evt = Event.current;
			switch (evt.GetTypeForControl(id))
			{
				...
				{ 
					if (GUIUtility.hotControl == id)
					{
						...
						value = (SnapValue(s_ValueDrag, snap, snapping) + 1.0f) * s_StartScale;
