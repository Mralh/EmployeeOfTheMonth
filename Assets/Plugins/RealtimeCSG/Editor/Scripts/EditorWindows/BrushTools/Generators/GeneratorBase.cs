...
namespace RealtimeCSG
{
	internal abstract class GeneratorBase : ScriptableObject//, IBrushGenerator
	{
		...
		public delegate Vector3 SnapToGridFunction(Vector3 worldPosition, CSGPlane plane, ref List<Vector3> snappingEdges, out CSGBrush snappedOnBrush, bool ignoreBrushes = false);
		...
		[SerializeField] protected Vector3[]	centerPoint = new Vector3[2];
		[NonSerialized] protected int[]			centerId	= new int[2];
		...
		public virtual void Reset() 
		{
			...
			centerPoint[0]			= MathConstants.zeroVector3;
			centerPoint[1]			= MathConstants.zeroVector3;
		...
		protected bool StartEditMode()
		{
			...
			centerPoint[1] = centerPoint[0] = ShapeSettings.GetCenter(gridTangent, gridBinormal);
		...
		{
			var direction		= //toGridQuaternion * 
									buildPlane.normal;
			transform.rotation = Quaternion.FromToRotation(MathConstants.upVector3, direction);// * fromGridQuaternion;
			transform.position = centerPoint[0];
		}
		...
		protected virtual void CreateControlIDs()
		{
			...
			centerId[0]	= GUIUtility.GetControlID (ShapeBuilderCenterHash, FocusType.Passive);
			centerId[1]	= GUIUtility.GetControlID (ShapeBuilderCenterHash, FocusType.Passive);
		}
		...
		protected void UpdateBrushOperation(float height)
		{
			if (forceCurrentCSGOperationType == invalidCSGOperationType)
			{
