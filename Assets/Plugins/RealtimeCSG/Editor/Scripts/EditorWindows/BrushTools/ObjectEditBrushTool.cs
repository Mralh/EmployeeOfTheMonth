...
namespace RealtimeCSG
{
	internal sealed class ObjectEditBrushTool : ScriptableObject, IBrushTool
	{
		...
		public void MoveCenter(Vector3 newCenter, bool registerUndo = true)
		{
			if (!Selection.activeTransform)
				return;
			...
			if (difference != Vector3.zero)
			{
				foreach(var topTransform in topTransforms)
				{
					topTransform.localPosition = topTransform.localPosition + difference;
				}
				UpdateTargetInfo();
			}
		}
		...
		public void HandleEvents(Rect sceneRect)
		{
			...
			try
			{
				...
				if (Selection.activeTransform != null)
				{
					if (Tools.current == Tool.Rotate)
					{
						if (worldSpacePivotCenterSet && HaveBrushSelection)
						{
							...
							{
								newPivot = PaintUtility.HandlePivot(newPivot, Tools.handleRotation, ColorSettings.BoundsEdgeHover, Tools.pivotMode == PivotMode.Pivot);
							}
					...
					if (Tools.current == Tool.Move)
					{
						if (HaveSelection)
						{
							...
							EditorGUI.BeginChangeCheck();
							{
								...
								newPosition = RealtimeCSG.Helpers.CSGHandles.PositionHandle(newPosition, Tools.handleRotation, snapping: doSnapping, snapVertices: snapVertices);
							}
							if (EditorGUI.EndChangeCheck())
							{
								MoveCenter(newPosition);
							}
						} else 
							snapVertices = null;
					}
				...
				switch (Event.current.type)
				{
					...
					{
						if (cloneDragKeyPressed && Keys.CloneDragActivate.IsKeyPressed()) { cloneDragKeyPressed = false; Event.current.Use(); break; }
